...
Connection: [See basic smbclient commands:](smbclient.md)
...
```bash
net view \\dc01 /all
# /all It will also list administrative shares (ending with the dollar sign)
```


### nbtscan

To identify NetBIOS information, such as NBT (NetBIOS over TCP)
...
### SMBClient: Protocol Negotiation Failed Troubleshooting
 ...
 [See smbclient basic commands](smbclient.md)
...
Go through a list of provided IPs:

```bash
while read ip; do enum4linux -a "$ip"; done < ips.txt
```
